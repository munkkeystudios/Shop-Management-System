...
const importController = require('../controllers/importController'); // <-- Ensure this is required
...
router.post('/sales', isCashier, saleController.createSale); // Manual Sale Creation
router.put('/sales/:id/payment', isCashier, saleController.updatePaymentStatus);
router.get('/sales/:id', isCashier, saleController.getSaleById); // Placed after specific /sales routes
...
// Reporting & Exports
router.get('/sales', isManager, saleController.getSales); // List all sales
...
router.post('/import/sales', isManager, importController.uploadSaleMiddleware, importController.importSales); // <-- NEW SALE IMPORT ROUTE
router.get('/import/sales/sample', isManager, importController.downloadSampleSaleTemplate); // <-- NEW SAMPLE DOWNLOAD ROUTE
...
// --- Test Route ---
router.get('/test', (req, res) => { // Auth applied, any authenticated user can access
