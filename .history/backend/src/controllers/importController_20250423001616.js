...
const xlsx = require('xlsx'); // Add xlsx for Excel support
const fs = require('fs');
    ...
    try {
        ...
        for (let i = 0; i < salesData.length; i++) {
            ...
            try {
                ...
                // **IMPORTANT**: Decide if importing historical sales should update current stock.
                // Generally, importing historical data should NOT change current stock levels.
                // If these imports represent NEW sales happening *now*, then uncomment the stock update.
                /*
                for (const item of validatedItems) {
                    await Product.findByIdAndUpdate(item.product, { $inc: { quantity: -item.quantity } });
                    // Add status update logic if needed
                    const updatedProduct = await Product.findById(item.product).select('quantity status');
                    if (updatedProduct && updatedProduct.quantity <= 0 && updatedProduct.status !== 'out_of_stock') {
                        await Product.findByIdAndUpdate(item.product, { status: 'out_of_stock' });
                    }
                }
                */
