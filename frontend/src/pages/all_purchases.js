...
import React, { useState, useEffect, useCallback } from 'react';
import { Search, Filter, FileText, FileSpreadsheet, Plus } from 'lucide-react';
import axios from 'axios';
        ...
        <div className="header">
          ...
          <div className="header-actions">
            ...
            <div className="action-buttons">
              ...
              <button className="create-btn" onClick={() => window.location.href = '/purchases/create'}>
                <Plus size={16} />
              ...
              <tbody>
                {purchases.map((purchase) => {
                  ...
                  return (
                    <tr key={purchase._id} onClick={() => window.location.href = `/purchases/${purchase._id}`}>
                      <td>{formatDate(purchase.date)}</td>
                      <td>{purchase.reference || purchase._id.substring(0, 8).toUpperCase()}</td>
                      <td>{purchase.supplier?.name || 'N/A'}</td>
                      <td>{purchase.warehouse?.name || 'Warehouse 1'}</td>
                      <td>
                        <span className={`status-badge ${getStatusClass(purchase.status)}`}>
