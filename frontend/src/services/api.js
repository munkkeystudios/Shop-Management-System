...
// Fallback to /api (useful for local dev with proxy)
...
// Response interceptor for handling errors/auth
api.interceptors.response.use(
  (response) => {
    console.log(
      `Incoming ${response.status} response from ${response.config.url}`,
...
// --- API exports remain the same --- //
// Auth API
export const authAPI = {
  login: (credentials) => api.post('/login', credentials),
  getProfile: () => api.get('/users/profile'),
  adminCreateUser: (userData) => api.post('/users', userData),
};
...
export const usersAPI = {
  ...
  delete: (id) => api.delete(`/users/${id}`), 
  exportUsers: (format = 'csv') =>
    api.get(`/users/export?format=${format}`, { responseType: 'blob' }),
};
...
export const salesAPI = {
  ...
  getLastBillNumber: () => api.get('/sales/last-bill-number'),
  exportSales: (format = 'csv', params = {}) =>
    api.get(`/sales/export`, { params: { format, ...params }, responseType: 'blob' }),
  importSales: (formData) =>
    api.post('/import/sales', formData, { headers: { 'Content-Type': 'multipart/form-data' } }),
};
